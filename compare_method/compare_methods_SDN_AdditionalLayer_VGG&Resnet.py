...
if __name__ == '__main__':
    ...
    parser.add_argument("--model", default="resnet18_CIFAR-100", type=str)
    ...
    #
    # for layer_inx in trange(5):
    #     IC_model = InternalClassifier(input_size[layer_inx], output_channels[layer_inx], args.class_num).to(device)
    #     IC_train(IC_model, data_loader=dataloader, layer_inx=layer_inx, epochs=args.n_step, nn_model=model)
    #     torch.save(IC_model, "../models/compare_model/SDN_{0}_layer{1}.pkl".format(args.model, layer_inx + 1))
    #
    #     AdditionalLayer_model = AdditionalLayerTemplet(input_size[layer_inx]**2*output_channels[layer_inx], args.class_num).to(device)
    #     AdditionalLayerTemplet_train(AdditionalLayer_model, data_loader=dataloader, layer_inx=layer_inx, epochs=args.n_step, nn_model=model)
    #
    #
    #     torch.save(AdditionalLayer_model, "../models/compare_model/AdditionalLayer_{0}_layer{1}.pkl".format(args.model, layer_inx + 1))
    ...
    for batch_idx, (sample, target) in enumerate(test_dataloader):
        ...
        for layer_inx in range(4):
            ...
            explain_model = torch.load(
                "../models/compare_model/{0}_{1}_layer{2}.pkl".format(compare_model, args.model, layer_inx + 1))
            ...
            for name, module in model.named_children():
                if name == f"layer{layer_inx+1}":
                    ...
                    break
            handle.remove()
            layer_prediction = F.softmax(explain_model(layer_features)[0])
        ...
        sample=sample.cpu()
        sample_show = sample.numpy().squeeze()
        #CIFAR10
        sample_show = sample_show.transpose(1, 2, 0)
        sample_show = (sample_show - np.min(sample_show)) / (np.max(sample_show) - np.min(sample_show))
        if pred.cpu() == target:
            plot_probs_of_layers(sample_show, np.array(layers_prediction),
                                 [str(c) for c in range(len(train_data.classes))], target,
                                 path=r"".format(compare_model,batch_idx))
        else:
            plot_probs_of_layers(sample_show, np.array(layers_prediction),
                                 [str(c) for c in range(len(train_data.classes))], target,
                                 path=r"".format(compare_model,batch_idx))
